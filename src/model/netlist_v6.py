...
@partial(jax.jit, static_argnums=(3, 4))  # size, n_nodes, dt, max_iter are static
def fixed_point_non_linear_solve(
    ...
    def iteration_step(carry):
        ...
        G, b = assemble_matrices(netlist_curr, size, n_nodes, dt, X_prev)
    ...
    init_carry = (X_prev, netlist, False, 0)
...
def assemble_matrices_with_non_linear_solve(
    ...
    time_step_size: float,
    X_prev: jnp.ndarray,
    max_non_linear_iter: int = 10,
    non_linear_tol: float = 1e-5,
):
    ...
    G, b = assemble_matrices(updated_netlist, size, n_nodes, time_step_size, X_prev)
